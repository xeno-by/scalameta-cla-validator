// NOTE: taken from https://github.com/typesafehub/ghpullrequest-validator/blob/master/src/main/scala/rest/github/api.scala
...
object Github {
  ...
  object Authenticate {
    val USER_AGENT = "github.com/xeno-by/scalameta-cla-validator"
   ...
   "note": "Authenticate API Access"
    ...
    def authenticate(user: String, pw: String): Authorization = {
      val previousAuth: Option[Authorization] =
        (getAuthentications(user,pw) filter (_.note == Some("Authenticate API Access"))).headOption
